...
my $database_host =        ""; # your database host, the same on phpBB uses
my $database_port =        ""; # your database port, the same one phpBB uses
...
my $to =                   ''; # email to address, must be single-quoted
my $from =                 ''; # email from address, must be single-quoted
...
(GetOptions( \%options,
    ...
    "u|url=s" => \$forum_address,
    "to=s" => \$to,
    "from=s" => \$from
)) || die "ERROR: Unknown arguments or parameters: %ARGV\n" unless ($ARGV < 0);

open FILE, ">", $queue_file or die $!;

my $database_name = "dbi:mysql:$database_name:$database_host:$database_port";
my $dbh = DBI->connect($database_name,$database_username,$database_password);
